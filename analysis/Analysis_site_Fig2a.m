...
path_m='./ERA5_sat_1980_2025_mon.nc';
...
%Below code until the next section can be changed to load('./ modern records at the Holocene record sites/sat_p.mat')
lon_m=ncread(path_m,'longitude');
lat_m=ncread(path_m,'latitude');
...
sat=zeros(length(target_lon),552);

    sat_temp=ncread(path_m,'t2m');    
    for k=1:length(target_lon)
    sat(k,:)=sat_temp(ii(k),jj(k),:);
    end
sat=sat-273.15;
